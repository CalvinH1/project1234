...
def search_venues():
  ...
  result = sess.query(Venue).filter(Venue.name.ilike(f'%{search_term}%')).all()
  data=[]
  today = str(datetime.now())
  for item in result:
...
def show_venue(venue_id):
  ...
  venue = sess.query(Venue).get(venue_id)
  shows= venue.shows
  today = str(datetime.now())
  past_shows =[{
...
def artists():
  data=[]
  artists = sess.query(Artist).with_entities(Artist.id, Artist.name).order_by(Artist.name).all()
  for artist in artists:
    data.append({'id': artist[0], 'name': artist[1]})
...
def search_artists():
  ...
  search_term=request.form.get('search_term', '') 
  result = sess.query(Artist).filter(Artist.name.ilike(f'%{search_term}%')).all()
  data=[]
...
def show_artist(artist_id):
  artist = sess.query(Artist).get(artist_id)
  shows= artist.shows
  today = str(datetime.now())
  past_shows=[{
